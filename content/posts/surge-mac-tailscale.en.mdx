...
3. **Add `underlying-proxy` only when you need it.** It usually connects fine without one, so it isn't required. The first connection may need a cold start, so give it a moment; only if it really won't connect, add `underlying-proxy` to `ts-home` and point it at an outbound policy:
   ...
   One more thing worth ruling out while you're here: if rules match and MagicDNS resolves to a 100.x address yet the connection still times out, check the Tailscale console for multiple leftover Surge nodes under the same name. Surge's built-in Tailscale is its own node identity, so repeatedly changing `auth-key` / `hostname` or reloading can leave old devices behind; removing the stale `surge-*` nodes and reconnecting rules out node-state confusion.
