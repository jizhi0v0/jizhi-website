...
3. **Add `underlying-proxy` only when you need it.** It usually connects fine without one, so it isn't required. The first connection may need a cold start, so give it a moment; only if it really won't connect, add `underlying-proxy` to `ts-home` and point it at an outbound policy:
   ...
   `underlying-proxy` is the right fix only when the control plane / DERP is **genuinely blocked**: the embedded node's cold-start handshake can't reach Tailscale's servers, so point `ts-home`'s underlying proxy at a policy with reliable egress and reload. But **don't jump to the wall** — "matches `Policy: ts-home` yet keeps timing out / working only intermittently" can equally be the embedded node's **IPv6 bug on an IPv4-only network** (where `underlying-proxy` does nothing and you want a `skip-proxy` bypass to the official client instead). First use [the checklist in Tailscale direct vs DERP](/en/posts/tailscale-direct-vs-derp) to tell "the network blocked it" from "the implementation broke."
...
- Only TCP-based control and DERP paths are supported — no UDP direct path (the reason, plus a real failure that's easily mistaken for the GFW but is actually the embedded node's IPv6 handling, is in [Tailscale direct vs DERP](/en/posts/tailscale-direct-vs-derp)).
