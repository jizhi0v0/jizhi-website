...
As of Surge Mac 6.7.0, Tailscale is built in as a proxy policy. You no longer run a separate Tailscale client — Surge itself joins the tailnet as a node, then routes matched traffic to machines inside it. Reaching internal devices, the NAS back home, a dev box — all go through this policy. Here's a minimal config you can copy verbatim.

> Requires Surge Mac 6.7.0+. Note that 6.7.0 (11310) currently ships on the **beta channel** — switch Surge's update channel to beta to receive it. iOS support is said to follow soon. It's still an early beta — only TCP-based control / DERP paths are supported, UDP direct paths are not, and the occasional crash is expected.
...
- `underlying-proxy="proxy"` wraps the control and DERP traffic in an outbound proxy; `"proxy"` points at the outbound policy below. The example is a Shadowsocks node — Surge uses comma-separated fields (`ss, host, port, encrypt-method=…, password=…`), not a raw `ss://` URL — so fill in your own values, or swap it for any other protocol's policy.

**`[Rule]`** decides what enters the tailnet:

- `DOMAIN-SUFFIX,tailXXXXXX.ts.net` matches MagicDNS names. Surge implements MagicDNS, but you still need an explicit rule to route it into the Tailscale policy.
...
3. **Add `underlying-proxy` only when you need it.** In the controlled test, switching it to `DIRECT` (no proxy) connected just as well — Snell Pro and Direct gave identical results, so it isn't required. It's just that Surge's Tailscale is still beta and the control / DERP handshake intermittently fails to connect; when it does stall, pointing it at a policy that gets out cleanly usually rescues it.
...
- Only TCP-based control and DERP paths are supported — no UDP direct path, so latency and throughput trail the native client.
- It's still beta and crashes now and then. It's enough as a lightweight, rule-driven entry into your internal network; for the full Tailscale experience, install the official client.
