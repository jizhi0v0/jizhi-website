...
3. **Prefer setting `underlying-proxy`.** Without an underlying proxy, control / DERP often fail to connect and the handshake stalls. Point it at an ordinary policy that gets out cleanly and the connection is far more stable.
