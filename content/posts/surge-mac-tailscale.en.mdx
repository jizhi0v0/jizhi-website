...
> Requires Surge Mac 6.7.0+. Note that 6.7.0 (11310) currently ships on the **beta channel** — switch Surge's update channel to beta to receive it. iOS has supported it since Surge iOS 5.102.0 (3730), though there the connection is lazy — it only dials the tailnet on your first request to a tailnet domain, so until then the node shows as not connected (not green) in the Tailscale console, turning green after a request; that's expected. It's still an early beta — only TCP-based control / DERP paths are supported, UDP direct paths are not, and the occasional crash is expected.
...
- It's still beta and crashes now and then. It's enough as a lightweight, rule-driven entry into your internal network; for the full Tailscale experience, install the official client — it coexists with Surge's built-in node just fine, with no extra config.
